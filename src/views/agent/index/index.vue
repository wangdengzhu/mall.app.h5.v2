    ...
    <div v-else>
      <div class="product-list" v-for="(item,key) in agentData" :key="key">
        ...
        <div class="product-pic">
          <img :src="item.spuwhitepic">
        </div>
        <div class="product-info">
          <p class="goods-info-tit">{{item.sputitle}}</p>
        </div>
        <div class="product-attr">
            <div class="goods-tit" v-if="item.agentskuinfos && item.agentskuinfos.length > 1">规格</div>
            <div class="goods-tit">销售价(元)</div>
            <div class="goods-tit">你能赚(元)</div>
        </div>
        <div class="product-attr" v-for="(val, i) of item.agentskuinfos" :key="i">
            <div class="good-spec" v-if="item.agentskuinfos && item.agentskuinfos.length > 1">{{val.skuattrnames}}</div>
  ...
  methods: {
    ...
    openLink() {
      ...
      if (!this.agentAgree) {
        ...
        Agent.agentGoods(this.shareid, allspu)
          .then(res => {
            Indicator.close();
            if (res.code === 1) {
              Toast("代理成功");
